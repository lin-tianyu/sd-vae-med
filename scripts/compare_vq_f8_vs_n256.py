...
fig.suptitle('Comparison between VQ-F8 and VQ-F8-n256 Models', fontsize=16, fontweight='bold')

# Scientific publication color palette - colorblind friendly
# Blue for standard model, Green for n256 model
colors = ['#0173B2', '#029E73']
...
for i, (metric, metric_label) in enumerate(zip(metrics, metric_labels)):
    ...
    for j, dataset in enumerate(datasets):
        ...
        if dataset in all_results[model1] and dataset in all_results[model2]:
            ...
            split_violin_data = {
                'Model': ['VQ-F8'] * len(data1) + ['VQ-F8-n256'] * len(data2),
            ...
            stats_data = {
                'VQ-F8': {'mean': np.mean(data1), 'median': np.median(data1), 'std': np.std(data1)},
                'VQ-F8-n256': {'mean': np.mean(data2), 'median': np.median(data2), 'std': np.std(data2)}
            }
            
            # Format text based on metric
            if metric == 'mse_2d':
                mean_diff = stats_data['VQ-F8']['mean'] - stats_data['VQ-F8-n256']['mean']
                # Add text with relative difference (%)
                rel_diff_percent = (mean_diff / stats_data['VQ-F8']['mean']) * 100
                ...
                mean_diff = stats_data['VQ-F8-n256']['mean'] - stats_data['VQ-F8']['mean']
                # Add text with relative difference (%)
                rel_diff_percent = (mean_diff / stats_data['VQ-F8']['mean']) * 100
...
fig.text(0.99, 0.01, 'VQ-F8 vs VQ-F8-n256 Comparison', fontsize=8, 
         color='gray', ha='right', va='bottom', alpha=0.7)

# Save the plot
output_file = os.path.join(output_dir, 'vq_f8_vs_n256_violin.png')
