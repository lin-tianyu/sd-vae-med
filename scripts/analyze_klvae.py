...
# Collect KLVAE models data - sort by downsampling factor for better visualization
kl_models = ['pt-kl-f4', 'pt-kl-f8', 'pt-kl-f16', 'pt-kl-f32']
# Extract the downsampling factor numbers for sorting
factors = [int(model.split('-f')[1]) for model in kl_models]
...
models_list = sorted(list(results_kl.keys()), key=lambda x: model_to_factor[x])
datasets_list = datasets.copy()

# Create a figure with 3 subplots
fig, axes = plt.subplots(1, 3, figsize=(18, 6), dpi=300)
fig.suptitle('KL-VAE Models Performance by Downsampling Factor', fontsize=16, fontweight='bold')
...
for i, (metric, metric_label) in enumerate(zip(metrics, metric_labels)):
    ...
    for j, dataset in enumerate(datasets_list):
        ...
        for model in models_list:
            if dataset in results_kl[model]:
                x_values.append(model_to_factor[model])
                y_values.append(results_kl[model][dataset][metric]['mean'])
                yerr_values.append(results_kl[model][dataset][metric]['std'])
    ...
    if metric == 'mse':
        ...
        for dataset in datasets_list:
            for model in models_list:
                if dataset in results_kl[model]:
                    y_values_all.append(results_kl[model][dataset][metric]['mean'])
                    yerr_values_all.append(results_kl[model][dataset][metric]['std'])
...
fig.text(0.99, 0.01, 'KL-VAE Model Comparison', fontsize=8, 
         color='gray', ha='right', va='bottom', alpha=0.7)

# Save the plot
output_file = os.path.join(output_dir, 'klvae_comparison.png')
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f"KL-VAE plot saved to {output_file}")
